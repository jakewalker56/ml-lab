...
GRID_PATH = '/Volumes/external/data/grid_sizes.csv'
TRAIN_WKT_PATH = '/Volumes/external/data/train_wkt_v4.csv'
THREE_BAND_PATH = '/Volumes/external/data/three_band'


def get_scalers(im_size, x_max, y_min):
    h, w = im_size  # they are flipped so that mask_for_polygons works correctly
    w_ = w * (float(w) / float(w + 1))
    h_ = h * (float(h) / float(h + 1))
...
# Load grid size
x_max = y_min = None
for _im_id, _x, _y in csv.reader(open(GRID_PATH)):
    if _im_id == IM_ID:
...
train_polygons = None
for _im_id, _poly_type, _poly in csv.reader(open(TRAIN_WKT_PATH)):
    if _im_id == IM_ID and _poly_type == POLY_TYPE:
        train_polygons = shapely.wkt.loads(_poly)
        break

"""
df = pd.read_csv(TRAIN_WKT_PATH)
df.head()
# Use just first image
polygonsList = {}
image = df[df.ImageId == IM_ID]
for cType in image.ClassType.unique():
    polygonsList[cType] = shapely.wkt.loads(image[image.ClassType == cType].MultipolygonWKT.values[0])
...
for p in polygonsList:
    for polygon in polygonsList[p]:
        mpl_poly = matplotlib.patches.Polygon(np.array(polygon.exterior), color=pyplot.cm.Set1(p*10), lw=0, alpha=0.3)
...
im_rgb = tiff.imread(THREE_BAND_PATH + '/{}.tif'.format(IM_ID)).transpose([1, 2, 0])
...
train_mask = mask_for_polygons(im_size, train_polygons_scaled)
...
pred_poly_mask = mask_for_polygons(im_size, pred_polygons)
