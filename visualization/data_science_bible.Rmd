...
# Goals
This is my attempt to explain data science principals in plain english.  It exists because I keep having to look up wikipedia pages that were unnecessarily obfuscationary, and wanted a clean and simple explanation for when I would inevitably forget specifics.

# Distributions
There are several distribution types that are worth knowing about, if for no other reason than that it makes reading technical documentation easier.  Below ore what I view as the most useful
...
A gaussian distribution is one in which 
...
# Cost Function
The [cost function](https://en.wikipedia.org/wiki/Loss_function#Expected_loss), or loss function, is the function that we are trying to minimize when we run a regression.  In other words, it defines what we mean when we say the model is a "good fit," and is closely related to the [deviance](http://stats.stackexchange.com/questions/6581/what-is-deviance-specifically-in-cart-rpart) (specifically, for linear models the deviance is equal to the sum of the squared residuals (or "errors"), often abbreviated RSS or SSE.  More on deviance later) 

A typical cost function would be the [mean squared error](https://en.wikipedia.org/wiki/Mean_squared_error), which is advantageous both because it is symetric (positive and negative misses look the same) and because it penalizes us for having outliers.  A more technical justification for least squares as a cost function is that minimizing least squares is equivalent to maximizing likelihood, assuming normal distributed errors.  See [here](http://cs229.stanford.edu/notes/cs229-notes1.pdf) for a proof of this, and see the likelihood section below for a discussion of what the heck that means.

In practice, if you set your cost function to something linear, you will get bad behavior out of more complicated regression models, because it creates a very flat local gradient curve, making it take much longer (and in some cases with asymtotic activation functions, impossible) to reach global optimas
...
As the name suggests, the Generalized Linear Model is a generalization of a linear regression model.  GLM assumes some distribution for errors (see Likelihood secion below), some linear function that maps inputs to a single numerical value (e.g. linear coefficients * input values), and a "link function."
...
The nice thing about GLM is that it lets us achieve non-linear behavior in our model, but still have the simplicity of only needing to estimate linear paramaters (e.g. estimate the coefficients B_0, B_1, B_2, etc.)

In the simplest case, you could just set the link function to f(x) = x, the identity function.  The inverse of this function is the same as the function itself, so f^-1(x) = f(x) = x.  In this special case, GLM become indistinguishable from a standard linear model.  You have some linear function of coefficients * inputs which maps to a value, and that value is your predictor for y.

In order to fit the coefficients of a GLM, we find the model that maximizes the likelihood (or log-likelihood) function for our response (see likelihood section below).  We typically use numerical optimization techiques for this such as gradient descent (see Numerical Optimization section below)

## Logistic Regression

But you can have a more complicated link function if you want! It turns out a logistic regression is just a GLM with a link function of XB = ln(u/(1 - u)) (called the "logit" function or the "log-odds" function), which gives an inverse link function of u = 1/(1+exp(-XB)).  When you need to make a prediction for y, you find the linear combination of B's and X's, and then you perform the inverse link function on your result to get your prediction.
...
In the above case, our linear mapping function is just the linear combination n = B_0 + x_1 B_1 + x_2 B_2.  The inverse link function is 1/(1+exp(-n)).

This has some nice properties.  It means that your response will always range from 0-1, which is nice because that's what probabilities do.  It also means our B's have a straightforward interpretation: B_i is the mulitpicative change in (log-?)**odds** (not probability) associated with a one-unit increase in X_i.  

**Note: need to figure out the right wording here... what is B exactly?

For example: if B_i is 1.5, then a 1 unit change in X corresponds with a 1.5 times increase in the odds of whatever you're predicting.  If your default odds are 1:10, then a 1 unit increase in X_i means your new odds are (1 + 1.5):10 (and your probability is 2.5/(2.5 + 10))

It's important to note here that the derivative of the inverse link function (lets call it g, so g = f^-1(X)) is given as g' = g(1-g) dg.  This is useful because some of the coefficient estimation techniques require us to know the slope of the the response with respect to the input variables.  Since it's really easy to calculate the derivative of the inverse logit function, it makes it particularly useful for these kinds of applications (see neural networks section below)

My big takeaway- and the thing that isn't really discussed very well in most places- is that **the logit function is totally arbitrary**.  It happens to have some nice properties- bounded between 0 and 1, easy to differentiate, etc. - but there's no a priori reason to think this is the optimal function for every data distribution.  It's really just a regular old GLM- it's mapping X's to numeric values, then transforming those numeric values to be between 1 and 0.  That's it.  No magic at all.

The biggest implication of this (to me, anyway), is that when you get a prediction from a logistic regression, that's our best estimate of the probability from a *linear model we built to estimate the log of the odds*.  If the log odds don't actually go linearly, then our model is going to be bad.  Full stop.  I've built models on data before that predict a probability of 10% given a common set of inputs, but then I look at the actual incidence for those inputs and it's more like 3%, and I'm confused- shouldn't my model be relatively close to the mean at every given point?  Well, there's no guarantee, no.  If you're using data that isn't highly explanatory, or if the relationship between X and log odds isn't linear, then a logit transform isn't going to do much good.
...
Notice that adding more observations to our training set *always makes our likelihood go down*.  This makes perfect sense- unless the probability of the next observation is exactly 1 or exactly 0, then it's always less likely that we'll see this current distribution *plus one extra observation* than it is that we'll see this current distribution.

When talking about [maximum-likelihood](https://en.wikipedia.org/wiki/Maximum_likelihood#Properties), you may sometimes see a formula written (with different variable names) as

l(M | x) = 1/n * SUM(ln(f(x | M)))
...
l(M | x) = 1/n * ln(f(x_1 | M) * f(x_2 | M) * f(x_3 | M)...)

In this case, l is actually the average [log-likelihood](https://en.wikipedia.org/wiki/Likelihood_function#Log-likelihood) function rather than simple likelihood, mostly because it has some useful properties when it comes time to start taking dirivatives. Since it is monotonically increasing with likelihood, it preserves ordering, and maximizing likelihood is equivalent to maximizing log-likelihood
...
3. The way we fit General Linear Models (see below) is by maximizing the likelihood.
